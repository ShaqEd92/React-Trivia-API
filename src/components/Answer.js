...
const Answer = (props) => {
  ...
  useEffect(() => {
    if (props.triviaData && props.triviaIndex >= 0) {
      ...
      let answers = [
        ...props.triviaData[props.triviaIndex].incorrect_answers,
        props.triviaData[props.triviaIndex].correct_answer,
      ];
  ...
  return (
    <Fragment>
      {props.started && (
        <div className="answers">{!props.gameDone && shuffledAnswers}</div>
      )}
    </Fragment>
  );
};
